...
const MostSoldChart = () => {
    ...
    const chartData = { type: "mostSold" as const, title: "Most Sold" }
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{ mb: { xs: 2, sm: 4, md: 6 }, width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <Typography variant="h1" className="dash-page-title" sx={{ fontSize: { xs: '1.4rem', sm: '1.6rem', md: '1.9rem', lg: '2.2rem', xl: '2.5rem' } }}>{chartData.title}</Typography>
            </Box>
            {!data ? (
                ...
                <Box sx={{ width: "100%", height: "100%" }}>
                    <DynamicChart data={data.mostSold} type={chartData.type} />
                </Box>
            )}
        </Box>
    )
